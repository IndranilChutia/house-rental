import { Button, CustomNextArrow, CustomPrevArrow, Footer, Navbar } from '@components';
...
import { PiCarLight, PiBathtub, PiIntersectSquareDuotone, PiArmchair } from "react-icons/pi";

const ownerImg = 'https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=2960&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
                ...
                <div className='w-full h-full grid lg:grid-cols-2 py-20'>
                    <div className='flex flex-col gap-8'>
                        <div className='flex flex-col gap-2.5'>
                            ...
                            <h1 className='text-lg md:text-xl lg:text-3xl font-semibold'>Let&apos;s Tour And See Our House!</h1>
                            <p className='text-xs md:text-sm text-slate-500 max-w-96'>Houses recommended by our partners that have been curated to become the home of your dreams!</p>
                        </div>
                        ...
                        <div className='h-full grid grid-cols-2'>
                            <div className='flex gap-4'>
                                <div className='h-16 w-16 rounded-full bg-slate-600 overflow-hidden'><img src={ownerImg} alt="owner" /></div>
                                <div className='flex flex-col gap-1.5'>
                                    <h3 className='font-semibold'>John Doe</h3>
                                    <button className='px-2 py-1 border-2 border-slate-600 rounded-full text-sm hover:bg-slate-100 transition-all'>Contact Owner</button>
                                </div>
                            </div>
                            <div className='flex flex-col gap-1.5'>
                                <p>Rent</p>
                                <div>
                                    <span className='text-3xl font-openSans text-slate-800 font-bold'>₹{amenities.rent}</span>
                                    <span> &nbsp;/ month</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className='order-first lg:order-last'>
                        <div className="slider-container">
                            ...
                            >
                                {data[0]?.images.map((image, index) => (
                                    <div key={index} className='h-24 w-24 rounded-md cursor-pointer overflow-hidden'>
                                        <img className='w-full h-full object-cover mx-2 rounded-md' src={image} alt="image" />
                                    </div>
                                ))}
                            </Slider>
                        </div>
