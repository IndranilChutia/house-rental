        ...
        <div className='h-full max-w-96 overflow-hidden rounded-md cursor-pointer'>
            <Link to={`/property/${id}`}>
                <div className='max-h-80 h-80 overflow-hidden rounded-xl w-full bg-slate-200'>
                    {thumbnail ? <img className="h-full w-full object-cover" src={generateImgLink(thumbnail.path)} alt={name} /> : <div className='w-full h-80 flex justify-center items-center text-7xl text-white'><IoMdImages /></div>}
                </div>

                <div className='my-3 mx-2'>
                    <h1 className='font-medium w-full truncate'>{name}</h1>

                    <h2 className='text-sm text-slate-600 mt-2.5'>from &nbsp;&nbsp;<span className='text-lg font-openSans font-bold tracking-wide'>₹{rent ?? "XX,XXX"}</span></h2>

                    <div className='flex items-center mt-1 text-slate-600'>
                        {/* Bedroom */}
                        <div className='flex items-center justify-center gap-1'>
                            <IoBedOutline />
                            <p className='text-xs text-slate-600'>{bedroom ?? 0} Bedroom</p>
                        </div>
                        {/* Dot */}
                        ...
                        <div className='flex items-center justify-center gap-1'>
                            ...
                            <p className='text-xs text-slate-600'>{carpetArea ?? 0} sq.ft</p>
                        </div>
                    </div>
